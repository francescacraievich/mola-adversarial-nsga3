...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="NSGA-II optimization for adversarial perturbations"
    )
...
def _create_evaluator(args, gt_traj, clouds, timestamps, tf_sequence, tf_static):
    ...
    generator = PerturbationGenerator(
        ...
        max_ghost_points_ratio=0.05,  # 5% ghost points
    )
...
def _print_header(args):
    ...
    print("  - Better Pareto front distribution than NSGA-II")
...
def main():
    ...
    try:
        ...
        if n_evals >= 5:  # Minimum threshold to save
            all_points = np.array(history_callback.all_fitness)
            valid_points = np.array(history_callback.valid_fitness) if n_valid > 0 else np.array([])

            np.save(numbered_output.with_suffix(".all_points.npy"), all_points)
            np.save(numbered_output.with_suffix(".valid_points.npy"), valid_points)
